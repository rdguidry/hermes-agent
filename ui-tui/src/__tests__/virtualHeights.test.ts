...
describe('virtual height estimates', () => {
  ...
  it('uses compound user prompt width when estimating user message wrapping', () => {
    ...
    expect(estimatedMsgHeight(msg, 27, { compact: false, details: false, termuxMode: false, userPrompt: '❯' })).toBe(3)
    expect(estimatedMsgHeight(msg, 27, { compact: false, details: false, termuxMode: false, userPrompt: 'Ψ >' })).toBe(4)
    expect(estimatedMsgHeight(msg, 26, { compact: false, details: false, termuxMode: true, userPrompt: '❯' })).toBe(3)
    expect(estimatedMsgHeight(msg, 26, { compact: false, details: false, termuxMode: true, userPrompt: 'Ψ >' })).toBe(4)
  })
